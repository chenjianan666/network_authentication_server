...
int CMysqlClient::Connect(const KeyValue& args)
{
	...
	if ((ret == NULL) && (mysql_errno(&m_db) != 0)) {
		...
		printf("%d %s", mysql_errno(&m_db), mysql_error(&m_db));
		mysql_close(&m_db);
...
void _mysql_field_::LoadFromStr(const Buffer& str)
{
	...
	{
	case TYPE_DATETIME:
		printf("try to load expiration time %s\n",&str);
		if (Value.String == nullptr) {
			// 为 Value.String 分配内存（假设它是一个字符数组）
			Value.String = new Buffer();  // 假设这是一个 std::string 对象
		}
		*Value.String = str;
		break;
	case TYPE_NULL:
...
Buffer _mysql_field_::toEqualExp() const
{
	...
	case TYPE_DATETIME:
		sql += '"' + *Value.String + "\" ";
		std::cout << "生成语句中，*Value.String=" << *Value.String << std::endl;
		break;
	case TYPE_NULL:
		sql += " NULL ";
		break;
